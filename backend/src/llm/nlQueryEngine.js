...
- If the retrieved records match the user's specific request (e.g. an amount or transaction ID), list each transaction with its Reference ID, status, and individual source amounts (Payment Gateway, Bank Statement, Orders Database).
    ...
    if (isExactAmountQuery && exactTargetAmount !== null) {
      if (transactions.length > 0) {
        const details = transactions.map(t =>
          `• Transaction **${t.txnRef}**: Status: **${t.status.replace(/_/g, ' ')}** (Priority: ${t.priority}) | Payment: ₹${t.paymentAmount ?? 'N/A'} | Bank: ₹${t.bankAmount ?? 'N/A'} | Order: ₹${t.orderAmount ?? 'N/A'}. ${t.explanation ? `AI Diagnosis: ${t.explanation}` : ''}`
        ).join('\n');
      ...
      answer = `Today's reconciliation rate is ${matchRate}% (${matchedCount} of ${total} transactions matched). There are ${exceptionsList.length} exceptions requiring review, driven by amount discrepancies and missing settlements.`;
    } else if (qLower.includes('biggest') || qLower.includes('largest')) {
      const topEx = transactions[0];
      if (topEx) {
        const topAmt = Math.max(topEx.paymentAmount || 0, topEx.bankAmount || 0, topEx.orderAmount || 0);
        answer = `The largest exception is transaction **${topEx.txnRef}** with an exposure of ₹${topAmt.toLocaleString('en-IN')}. Status: **${topEx.status.replace(/_/g, ' ')}**. ${topEx.explanation || 'Requires manual review.'}`;
      } else {
        answer = 'No exceptions found for this batch.';
      }
    } else if (transactions.length > 0) {
      answer = `Found ${transactions.length} matching transaction(s) for your inquiry. Key transactions include: ${transactions.slice(0, 3).map(t => `${t.txnRef} (${t.status}, ₹${Math.max(t.paymentAmount || 0, t.bankAmount || 0, t.orderAmount || 0).toLocaleString('en-IN')})`).join(', ')}.`;
    } else {
